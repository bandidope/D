    ...
    try {
        ...
        try {
            ...
            if (user) {
                ...
                global.db.data.users[m.sender] = {
                    exp: 0, limit: 10, registered: false, name: m.name, age: -1, regTime: -1,
                    afk: -1, afkReason: '', banned: false, useDocument: true, bank: 0, level: 0,
                }
            ...
            if (chat) {
                ...
                global.db.data.chats[m.chat] = {
                    isBanned: false, bienvenida: false, antiLink: false, antilinkxxx: false,
                    detect: true, onlyLatinos: false, nsfw: false, audios: false,
                    modoadmin: false, expired: 0, antiLag: false, per: [],
                }
            ...
            if (settings) {
                ...
                self: false, autoread: false, antiPrivate: false, antiPrivate2: false,
                antiBot: true, antiSpam: true, status: 0
            }
        ...
        if (opts['que'] && m.text &&!(isMods || isPrems)) {
            let queque = this.msgque, time = 1000 * 5
        ...
        const user = m.isGroup? participants.find(u => normalizeJid(u.jid) === senderNum) : {}
        const bot = m.isGroup? participants.find(u => botNums.includes(normalizeJid(u.id && u.jid))) : {}
        ...
        for (let name in global.plugins) {
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                if (gruposPermitidos.includes(m.chat) &&!comandosPermitidos.includes(command)) return;
                ...
                if (plugin.premsub &&!isPremSubs) { fail('premsubs', m, this); continue }
                ...
                let extra = { match, usedPrefix, noPrefix, _args, args, command, text, conn: this, participants, groupMetadata, user, bot, isROwner, isOwner, isRAdmin, isAdmin, isPremSubs, isBotAdmin, isPrems, chatUpdate, __dirname: ___dirname, __filename }
                try {
                    ...
                    if (m.limit) conn.reply(m.chat, `Utilizaste *${+m.limit}* ✳️`, m, rcanal)
                }
                break
            }
        }
...
global.dfail = (type, m, conn, usedPrefix) => {
    let msg = {
        rowner: "> *Sorry Bro Este Comando Solo Lo Puede Usar Mi Owner 🥴*",
        owner: "> *Sorry Bro Este Comando Solo Lo Puede Usar Mi Owner 🥴*",
        mods: "> *Lo Siento Solo Lo Pueden Usar Los Moderadores 🫂*",
        premium: "> *Necesitas Ser Usuario Premium Para Usar Esta Función 🪄*",
        premsubs: '> *Esta función solo puede ser usada por subbots premiums* 😆',
        group: "> *Comando Creado Únicamente Para Grupos 🥰*",
        private: "> *Debes Usar En El Privado Del Bot El Siguiente Comando 🌟*",
        admin: "> *Tu No Eres Admin JAJA 😂*",
        botAdmin: "> *Debo Ser Admin Primero Para Ejecutar La Función 🤝*",
        unreg: "> *Debes Registrarte Primero Con.reg nombre.edad 🫂*",
        restrict: "> *Comando Desactivado*"
    }[type]
